...
public class compactmoney implements CommandExecutor {
        ...
        if (sender.hasPermission("redcore.moneygive")) {
            if (args.length > 1) {
                ...
                try{
                   ...
                   // sender.sendMessage(prefix + plugin.getConfig().getString("wrong-formatting").replaceAll("&", "§").replaceAll("%prefix%",prefix));
                    return false;
                }
                if (p != null && quantita != null) {
                    ItemStack soldi = item.soldi("compatti");
                    int i;
